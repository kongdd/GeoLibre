...
- [x] Field Collection tool for capturing point, line, and polygon observations with a per-layer custom form, repeatable notes, and up to ten inline photos (10 MB total) per observation, placed by device GPS or by tapping the map, with a floating quick-open control; captures are written to a tagged GeoJSON layer that flows into the attribute table, export, and offline use
...
## v2.6: Data and style deep links, an MCP server for AI clients, COG-backed terrain, and a design system of its own (current)
