...
- `GEOLIBRE_STORAGE_PATH`: local object directory, default `./data`. Each project
  is stored under `projects/<id>/`; inline PNG/JPEG/WebP/GIF/AVIF images are
  extracted into that project's `assets/` directory.
- `GEOLIBRE_ANONYMOUS_PROJECTS=1`: allow token-free project storage in one shared
  workspace. This intentionally provides no user or project isolation.
